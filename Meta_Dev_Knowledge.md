...
改寫——本準則生效後才是「新文案的預設要求」；既有文案是否改寫，依
Phase BB 稽核表逐筆判斷（多數僅需覆核，不需大改）。

1. **口吻統一**：一律第二人稱直接對話（「你」），不用「使用者」「用戶」
   等第三人稱指稱訪客本人；不用過度熱情或行銷腔（禁用「立即」「馬上」
   「趕快」等催促語、驚嘆號濫用）；語氣沿用既有陪伴/教練調性（外事局
   指南、FAQ 已是此調性的既有範例，可直接參考）。
2. **簡潔**：能一句講完不用兩句；避免「進行○○的動作」「針對○○做
   處理」等贅語結構，改用直接動詞（例：「進行搜尋」→「搜尋」）。
3. **結構**：說明性文案優先「結論先行＋要點」，比照 PAT-152（加油站
   卡片格式全站化）已建立的卡片格式標準，延伸適用至頁面級文案（不限
   於卡片內文）。
4. **直接點出重點與使用方式**：功能說明類文案（空狀態、頁面標題副
   文、按鈕輔助文字）需同時包含「這是什麼」＋「怎麼用」兩要素，不可
   只寫「這是什麼」留使用方式讓使用者自行摸索。
5. **內附工具/連結需明確標註**：頁面若依賴外部資源或站內其他功能
   （如「查看完整說明」「前往資源」），連結文字本身需可預期會導向
   什麼，不用「點此」「了解更多」等無資訊量措辭。

**明確排除，不適用本準則**（既有稽核已建立的專有名詞/法規原文/事實
性內容）：外事局指南（Phase AT/AT.c 已定案的事實性內容）、隱私政策
的權利義務條款本文、GDPR 條款編號、Fiktionsbescheinigung 等德文專有
名詞的既有翻譯慣例（見 PAT-37/39 全站德文首次原則）、資源分類卡片
內文（PAT-152 已建立獨立的格式標準，不套用本準則的語氣細則）。
...
## PAT-162 [CORE_IMMUTABLE]: 全站文案準則建立 + 文案語氣違規的判斷框架

**準則本身**：見本檔最前方獨立章節「文案準則」（Phase BB 建立）。以下
記錄稽核過程中確立的**判斷框架**，用於未來新增文案時快速判斷是否落入
準則的規範範圍，避免誤傷不該套用的內容類型。

**核心區分：「內容分類標籤／作者署名」vs「對讀者說話的散文」**——
「使用者」一詞禁用的對象是後者，不是前者：
- 允許：以「使用者」作為**內容類型標籤或第三方作者署名**，例如「使用者
  提交」「使用者建議」「使用者推薦」徽章（`UserSubmissionsList.tsx`、
  `Schools.tsx`、`Recommendation.tsx`、`RecommendationCategory.tsx`、
  `SchoolDetail.tsx` 皆屬此類）、「匿名使用者」「已刪除的使用者」等
  第三方顯示名稱佔位字串（`ReviewList.tsx`、`deleteAccount.ts`）、
  提及「其他使用者」指涉讀者以外的第三人（`Board.tsx` 追蹤功能說明）。
  這些是名詞片語標籤，不是對讀者本人說話。
- 禁止：以「使用者」取代「你」，用第三人稱敘述讀者本人能做什麼，例如
  原公告文案「使用者可於「我的貼文」隨時刪除⋯」（已改為「你可於⋯」）。
  這是散文中對讀者直接說話卻用了第三人稱指稱。
- 邊界案例：`MyProfile.tsx` 的「第 N 位註冊使用者」判定為**統計徽章
  標籤**（同「使用者提交」的名詞片語性質），非對讀者的散文稱呼，故
  維持不改；隱私政策（`Privacy.tsx`）等法律文件本身已由準則明文排除，
  不受此框架影響。

**其他確立的類型排除**：導覽列／下拉選單／分頁籤等短詞彙標籤是與
「說明性文案」不同的文類，不需套用「結論先行＋要點」「what+how」的
完整散文結構要求；已存在的「查看全部 →」類連結搭配鄰近標題語境時視為
足夠明確，不需為單一實例另立更嚴格的措辭標準造成全站不一致。

**本輪稽核結果**：全站現有文案絕大多數已符合準則（推測因先前各 Phase
已有機地朝口語化、第二人稱風格靠攏）。實際改寫僅 4 處：
`errorMessages.ts` 的 `PGRST204` 訊息移除工程術語「payload」；
`Schools.tsx` 篩選空狀態補上「試試調整篩選條件」的怎麼做；
`announcements.json` 2026-06-25 舊公告「使用者」改「你」（比照 Phase AW
先例，公告內容明列於本輪範圍內，歷史條目可編輯）；`Home.tsx` 完成
訊息移除「準備好了嗎？」的贅字反問句。

## PAT-163 [CORE_IMMUTABLE]: 貼文卡片共用元件 + 讀取查詢重試工具（讀可重試、寫不可重試的邊界）

**BC.a · `ListingCardBody.tsx`**：`BoardList.tsx`（討論區全頁列表）與
`HotBoardCarousel.tsx`（首頁熱門討論區）共用同一份貼文渲染邏輯（badge/
標題/內文/照片），避免兩處分別維護一份「貼文長相」定義。以
`truncateDescription`/`showPhotos`/`showExpiry` 三個 boolean prop 控制
差異，而非另寫一份摘要卡元件。截斷手法選用 CSS `line-clamp-4` +
client-side `expanded` state 切換，而非沿用站內既有的 `<details>`
展開模式（`FAQ.tsx`／`SchoolDetail.tsx`／`MyProfile.tsx` 等大量使用）——
`<details>` 適合「預設收合、點擊才顯示全新一塊內容」（如 FAQ 詳解、
刪除帳號說明），本情境是「同一段自由文字被截斷、展開後接續同一段」，
用 `scrollHeight > clientHeight` 量測是否真的溢出，只在確實截斷時才
顯示「展開查看更多」按鈕，避免短文也顯示無效果的按鈕；量測 effect 的
依賴刻意不含 `expanded`，否則展開後 clientHeight 等於 scrollHeight 會讓
按鈕在展開當下立即消失。卡片包在 `<Link>` 內時，展開按鈕的 onClick 需
`preventDefault()`+`stopPropagation()`，否則點擊會誤觸發外層 Link 導航。

**BC.b · `fetchWithRetry.ts`**：全站讀取查詢（`supabase.from(x).select(y)`）
的統一重試工具，3 次總嘗試（1 次原始 + 2 次重試）、退避 500ms→1000ms、
`console.warn`（非 `console.error`，避免誤觸發測試工具的錯誤偵測）記錄
格式 `[fetchWithRetry] table=X attempt=N/3 status=S retryable=R final=F
error=...`。

**關鍵前置發現（動手實作前先查證 `node_modules/@supabase/postgrest-js`
原始碼，而非直接假設要從零蓋重試邏輯）**：本專案安裝的
`@supabase/postgrest-js@2.110.2` 對 GET/HEAD/OPTIONS 請求本身就內建重試
（`retryEnabled` 預設 `true`），對象是「fetch 本身丟出例外」與「HTTP
520/503」兩種情況，退避 1s→2s→4s（封頂 30s）、預設最多 3 次。若不處理，
會與本工具疊加成最壞 9 次嘗試，直接超出「總共 3 次、5 秒內判定完成」的
設計預算。解法：每個呼叫點的 query chain 尾端一律加上 `.retry(false)`
（該套件公開提供的鏈式方法），把所有重試邏輯統一收斂到 `fetchWithRetry`
一處控管，不使用套件本身的內建重試。

**可重試 vs 不可重試的判斷依據**：postgrest-js 的已解析回應物件（非拋出
例外時）帶有公開的 `status` 欄位（`PostgrestResponseBase.status`），
`status === 0` 代表 fetch 本身失敗被轉換為已處理的錯誤（非拋出例外，
不需要 try/catch）；`status >= 500` 為伺服器暫時性錯誤。其餘（4xx，含
RLS `42501`、登入過期 `PGRST301`、重複鍵 `23505` 等）一律不重試。此判斷
比 postgrest-js 內建的「僅認列 503/520」更寬（依 Lily 指示的「5xx 皆
重試」），但**僅套用於讀取**——不得比照套用於任何 `.insert()`/`.update()`/
`.delete()` 呼叫，重試寫入有造成重複寫入的風險，此邊界為 Lily 明確指示，
不得因「順手一致」擴大範圍。

**盤點與套用範圍**：全 repo 26 個 `.select(` 讀取呼叫點，除
`src/lib/types.ts` 的 `attachProfiles`（受保護 4 檔之一，未經授權不得
修改）外，其餘 25 處全數套用。依失敗後行為分兩類：(1) 頁面主要內容
讀取（`Board.tsx` 貼文/評價、`SchoolDetail.tsx`、`UserSubmissionsList.tsx`、
`MyProfile.tsx`、`CommentSection.tsx`）新增 `loadError` 旗標，重試耗盡後
渲染既有 `EmptyState` 元件搭配新文案「讀取失敗」+「請檢查網路連線後
重新整理頁面。」（沿用 PAT-162 文案準則的 what+how 結構，不與「目前
沒有內容」的真空狀態文案混淆）；(2) 裝飾性/衍生統計資料
（`useFollow.ts`／`useBadges.ts`／`useContributions.ts`／
`workflowProgress.ts`／`DeletionRestoreBanner.tsx`／`badges.ts`／
`useLikes.ts`／首頁兩個熱門輪播 `useHotSchools.ts`/`useHotListings.ts`）
僅套用重試、維持既有的靜默失敗行為（沿用原本就沒有可見錯誤 UI 的
既有行為，不強加新的錯誤畫面）。

## PAT-164 [CORE_IMMUTABLE]: user_submissions 狀態生命週期 + 分類專屬篩選欄位的 schema 擴充模式

**user_submissions.status 的 4 值語意**（Phase BD 查證）：CHECK 約束為
`pending`/`approved`/`rejected`/`archived`，但 `UserSubmissionsList.tsx`
的查詢僅 `.in('status', ['pending', 'approved'])`——代表 `approved` 並
**不是**「已完全處理、可從清單移除」的終態，只是「內容審核通過、可
公開顯示」，仍會持續出現在「使用者建議」列表。真正讓項目從清單消失
的是 `rejected`（不採用）或 `archived`（已處理/已收錄，不需再關注）。
**教訓**：查到某筆記錄的 status 已經是非 `pending` 的某個值，不能直接
假設「已經處理完了」——要對照該 status 值在前端實際查詢邏輯裡的作用，
是否真的達到「不再需要人工關注」的效果。Phase BD 的洪堡學院 Konstanz
提交記錄查到時已是 `approved`（非預期中的 `pending`，原因不明——可能
是先前某次審核動作，或提交當下的預設值），但因為 `approved` 仍會顯示
在清單中，故仍準備了 `UPDATE ... SET status = 'archived'` 交給 Lily
執行，而非誤判「已是 approved 所以不用管」。

**分類專屬篩選欄位的 schema 擴充模式**：`Recommendation` 這個跨 9 個
分類共用的資料型別，新增僅 `housing` 分類使用的 `fee_status`/`term`/
`target` 三個篩選欄位時，直接加為**選填**屬性（`?:`），而非另外開一個
`HousingRecommendation extends Recommendation` 子型別或另建
`housing.ts` 專屬檔案——其他 8 個分類的資料完全不受影響（欄位留空即
可），共用元件（卡片渲染）也不需要為了少數分類專屬欄位而改變介面。
這個模式可複用於未來任何「多數分類共用同一型別，其中一兩個分類需要
額外結構化欄位」的情境：優先選填欄位擴充，只有當專屬欄位多到讓共用
型別難以閱讀、或該分類的資料形狀已經明顯偏離共用型別的核心意圖時，
才考慮拆分子型別。

**篩選元件的單選/複選判斷**：沿用 `Schools.tsx` 既有的 `<select>`
單選下拉（`fee_status`/`term`，一次篩一個值），但 `target` 因資料本身
是陣列（同一平台可對應多個族群）且指令書明確要求「可複選」，改用
`Board.tsx` 既有的 filter chip **視覺樣式**（`border-brand-burgundy`
高亮）但**選取邏輯**改為獨立開關（`Set`/陣列 toggle，而非單選互斥）。
這不算「重新設計一套新互動」——沿用的是既有的視覺語言，只有選取狀態
的資料結構因應「單選 vs 複選」的實際需求而不同，兩者本來就該不同，
勉強套用單選元件處理複選需求才是真正的削足適履。

## PAT-165 [CORE_IMMUTABLE]: 「大分類→子板塊」兩層導覽模式（首次先例，德文學習分類）

**問題背景**：`RecommendationCategory.tsx` 原本服務 9 個分類，每個分類
內部是單層扁平清單（最多疊加篩選，如 Phase BD 的找房 fee_status/term/
target）。Phase BE 新增的「德文學習」分類需要「大分類→子板塊」兩層
結構（9 個子板塊：文法/綜合/詞彙/字典/寫作/閱讀/聽力/口說/檢定），
這是全站首次出現的兩層導覽需求。

**架構決策（先讀既有機制，判斷是否複用 vs 設計最小侵入方案）**：
1. 大分類層沿用既有的 `/recommendation/:slug` route（slug=
   `german_learning`），與其餘 9 分類走同一套路由，零新增路由框架。
2. 子板塊層**沒有**現成可直接複用的機制——`Board.tsx` 的
   mainFilter/subFilter 雖是類似的階層式篩選，但只有 `viewMode`／
   `mineSubTab` 兩個維度會同步進 URL query param（`?view=`/`?sub=`），
   mainFilter/subFilter 本身不進 URL。故子板塊選擇改為**延伸**（非
   複製）Board.tsx 已證明可行的 `useSearchParams` 技術：同一個
   `?sub=` 參數名稱、同樣的「初始化時讀取 + useEffect 監聽變化」模式，
   額外新增「點擊 tab 時呼叫 `setSearchParams` 寫回 URL」，讓分享連結
   能直接回到指定子板塊（Board.tsx 原本只讀不寫，因為它的深連結入口
   來自外部導覽而非頁內切換；德文學習的子板塊切換發生在頁內，需要
   雙向同步）。
3. 兩層結構的複雜度（9 tab + 3 篩選維度）明顯高於其餘單層分類，抽成
   獨立元件 `GermanLearningBoard.tsx`，由 `RecommendationCategory.tsx`
   僅在 `slug === 'german_learning'` 時整段替換渲染，不塞進通用渲染
   路徑——避免其餘 9 個簡單分類的程式碼被迫承擔不屬於它們的複雜度。
4. 子板塊本身是「類別」維度的導覽（非篩選項），資料在來源表格裡對應
   「類別」欄位，且**同一資源可橫跨多個子板塊**（例：
   Deutschlernerblog.de 同時屬於文法/詞彙/閱讀/聽力/寫作/檢定 6 個
   子板塊）——故 `board` 欄位設計為陣列而非單一值，子板塊切換即是
   `item.board?.includes(currentBoard)` 的篩選，與其餘「等級／適合
   族群／狀態」三個篩選維度並行疊加。

**狀態的視覺分級**：「非正常運作」的 4 種狀態（已變更/內容老舊/需人工
確認/停更可用）依「是否仍可正常使用」分兩級視覺提示，而非一律套用
同一種警示樣式——「已變更」「停更可用」代表資源仍可用只是性質改變，
用溫和的金色提示；「內容老舊」「需人工確認」代表可靠度有疑慮，用
較強烈的警示色（`state-danger`）。零虛構原則要求狀態差異必須一眼
可辨，但不代表所有非正常狀態都該用同一種「警告」語氣呈現——過度
一致的警示反而會讓使用者對真正需要注意的項目降低敏感度。

**複用模式適用範圍**：未來若有第三層（例如子板塊內還要再分小分類），
這套「route 存大分類、query param 存子層、獨立元件封裝複雜度」的
模式可以再往下一層延伸；但若只是單層清單加篩選（如找房分類的模式），
不需要這套機制，直接沿用 Phase BD 的做法即可。

## PAT-166 [CORE_IMMUTABLE]: 狀態枚舉值零使用不等於該移除定義 + 「不預先篩選、讓標籤說話」的設計方向調整

**背景**：Phase BF 剔除 German-Learning-Online.de（德文學習分類「需
人工確認」狀態的唯一一筆）後，`GermanLearningStatus` 型別與
`GERMAN_LEARNING_STATUS_LABEL` 對應的 `unconfirmed` 值全資料集零使用。

**決策：保留列舉定義，不因零實例而移除**。理由：狀態列舉是通用詞彙
系統（正常運作/已變更/內容老舊/需人工確認/停更可用），不是為單一
資源量身打造的特例欄位——`unconfirmed` 描述的是「網站拒絕自動化
存取、需人工確認」這個明確會再發生的情境類別（未來新增資源時很可能
再遇到同樣的拒爬蟲網站），移除定義只是為了消除當下零使用的型別而
過度反應，之後若再需要，还要重新定義一次，並且要重新決定視覺分級
（見 PAT-165 的兩級提示系統，`unconfirmed` 目前歸在強警示級）。
**規則**：列舉值使用歸零時，先確認這是「通用詞彙的偶然巧合」還是
「這個值從一開始就是為了單一特例而設」——前者保留定義只移除資料，
後者才需要連定義一起清理。

**設計方向調整：從「預先篩選保護使用者」轉為「完整顯示＋標籤自行
判斷」**。Phase BE 原始設計是「狀態篩選預設僅顯示正常運作」，出發點
是不讓使用者不小心點進已停用/不可靠的資源。Phase BF 推翻這個預設，
理由：
1. 狀態徽章（PAT-165 的兩級視覺分級）已經足夠顯眼，篩選機制是重複
   保護，且「預設隱藏」反而讓使用者不知道這些資源存在，喪失知情
   選擇的機會（例如「停更可用」的資源可能仍是使用者想要的複習素材，
   不該被預設篩掉）。
2. 這個方向與「子板塊新增全部選項並設為預設」是同一個設計邏輯的
   延伸：兩者都是把「要不要縮小範圍」的決定權還給使用者，而不是
   由系統預先決定一個未必符合每個使用者需求的起始狀態。
**規則**：當一個「保護性預設篩選」與「已有明顯視覺標示」功能重疊時，
優先讓視覺標示做完整的溝通工作，篩選機制留給使用者主動操作，不要
兩層都做且都預設啟動——那是防禦性設計的過度疊加，代價是犧牲資訊
的完整呈現。

## PAT-167 [CORE_IMMUTABLE]: 兩個相似兩層導覽需求時，優先複製而非抽共用元件——判斷依據

**背景**：Phase BG 的 DACH 實習/求職分類與 Phase BE/BF 的德文學習分類，
都是「大分類→子板塊＋多維篩選」的兩層結構，指令書明確要求先評估能否
把 `GermanLearningBoard.tsx` 泛化為共用元件。

**決策：複製並改寫（新建 `CareerBoard.tsx`），不泛化**。判斷依據：

1. **真正共通的部分很小**：兩者唯一完全相同的技術只有「tab bar +
   `useSearchParams` 同步」這一段（約 15 行）；子板塊數量（9 vs 2）、
   篩選維度數量與型態（等級單選+族群複選 vs 費用/模式/國家單選+族群
   複選，2 維 vs 4 維）、是否有狀態徽章系統（有 vs 無）皆不同。
2. **只有 2 個具體案例，尚不足以判斷正確的抽象邊界**。若現在就把
   「N 個篩選維度、每個維度可選單選/複選、可選配徽章系統、可選配
   『全部』分頁」全部塞進一個共用元件的 props/config API，這個 API
   設計本身就是在為「假設中的第 3 個案例」猜測需求——猜對的機率不高，
   猜錯的代價是兩個現有案例都要遷就一個彆扭的共用介面，且下一個案例
   如果形狀又不同，還是得再改一次這個共用元件。
3. **與 BE.a 建立的既有判斷一致**：BE.a 當初面對「要不要把 Phase BD
   找房頁的行內篩選邏輯泛化」這個類似問題時，選擇的也是新建獨立元件
   （`GermanLearningBoard.tsx`）而非改造共用。這次是同一個判斷邏輯的
   延續，非重新評估後恰好又得到同樣結論。
4. **複製的成本遠低於抽錯象的成本**：重複約 15 行「tab+URL 同步」的
   技術性樣板，之後若要一次修正兩處（例如發現這個技術本身有 bug），
   成本是「改兩個檔案」；而抽出來的共用元件如果邊界抽錯，成本是
   「拆掉重抽＋遷就期間兩邊都將就一個不合身的介面」。

**規則**：兩個功能表面上結構相似（都是「兩層導覽＋篩選」）不代表現在
就該抽共用元件——先確認「相似的是核心技術本身」還是「相似的只是
需求敘述的形狀」。當篩選維度的數量/型態/是否需要額外系統（如徽章）
每個案例都不同，通常代表現在還在累積案例的階段，先各自實作、只複製
明確可重複的技術片段（此處是 tab+URL 同步），等第 3、4 個案例出現、
真正共通的部分變得清楚時，才是抽共用元件的合理時機。

## PAT-168 [方法論]: 批量高度命名相似的網域，逐一查無實據應懷疑整批為 AI 幻覺而非零星疏漏

**背景**：Lily 提供的 DACH 實習/求職清單第三輪查證中，發現「奧地利
職缺聚合站」分類下有 24 個網域（Jobkurse.at、Jobsuchmaschine.at、
Jobturbo.at 等），命名模式高度一致（皆為「Job + 英文字根 + .at」），
逐一搜尋後**全數查無對應網站存在**。

**方法論**：當一批項目呈現以下特徵組合時，應該懷疑整批是 AI 生成
（或人工彙整時誤信 AI 生成清單）的幻覺內容，而非「剛好這麼多個都
是真實存在但目前失效/搬遷」的零星巧合：
1. **命名高度模板化**：同一批項目的名稱遵循肉眼可辨的固定公式
   （此案例是「Job+字根+固定字尾」），像是某種生成規則的產物，而非
   真實世界品牌命名通常會有的隨機性與差異化考量。
2. **逐一查證零命中**：對批次中每一個項目個別搜尋，得到的不是「找到
   但已停用/搬遷」（那是真實曾存在的證據），而是「完全查無此網域/
   網站」（沒有任何歷史痕跡、註冊記錄、快取版本可循）。
3. **批量出現在同一分類/來源**：這些項目集中出現在原始資料的同一個
   小節，暗示它們可能來自同一次生成過程（例如請 AI「列出更多同類
   平台」時，AI 在真實項目之外編造了看似合理的填充項目）。

**規則**：符合以上特徵組合時，**整批判定為疑似幻覺、一次性剔除**，
不需要對每一個都個別寫「查無實據」的理由（因為理由都相同），但仍需
在剔除紀錄中列出完整名單以利日後追溯，且要跟「真實存在但已停用」
（如本案例中的 ERASMUSINTERN，有明確公告可查證）與「網址誤植但實體
存在」（如本案例中的 Jobscout24.at 實際併入 jobscout24.ch）這兩種
情況明確區分——後兩者是個別查證後的具體結論，不能跟批量幻覺判定
混為一談。**這個方法論本身獨立於任何特定功能實作**，未來查證任何
...
## PAT-170 [CORE_IMMUTABLE]: 15 分鐘編輯窗須由 RLS 強制（前端隱藏僅為體驗優化）+ 既有明確產品決策的推翻須先向 Lily 確認

**背景**：Phase BI 為 `listings`（討論區貼文）與 `school_reviews`（語校
評價）新增 15 分鐘編輯窗。查證現況時發現兩個關鍵落差：`listings` 已有
`listings_own_update` policy 但**完全沒有時間限制**（任何時候都可編輯，
非本輪才出現的既有缺口）；`school_reviews` **完全沒有 UPDATE policy**
（RLS 預設全擋，先前設計是「只能刪除重發」）。兩表也都沒有 `updated_at`
欄位。

**安全設計規則**：15 分鐘限制必須寫進 UPDATE policy 的 `USING` 子句本身
（`auth.uid() = user_id AND created_at > now() - interval '15 minutes'`），
不能只靠前端隱藏「編輯」按鈕——否則使用者能繞過 UI 直接呼叫 API 在超過
時限後仍修改。`listings` 用 `ALTER POLICY ... USING (...)` 直接改寫既有
policy（角色/命令不變，只換條件）；`school_reviews` 因原本零 UPDATE
policy，改用 `CREATE POLICY`，角色沿用該表既有 INSERT/DELETE policy
的 `public`（非 Phase BH card_ratings 新立的 `authenticated` 慣例——沿用
每張表自己既有的角色設定，而非強推全站新標準）。

**RLS 驗證方法論**：`execute_sql` 預設以特權角色執行，直接下
UPDATE 測不出真實 RLS 行為。改用
`SET LOCAL role = 'authenticated'; SET LOCAL request.jwt.claim.sub = '<uuid>';`
在交易內模擬指定使用者的 JWT 身分，再執行 UPDATE...RETURNING——
回傳 0 筆代表 RLS 正確擋下，回傳 1 筆代表允許。本輪以此法驗證三種
情境：20 分鐘前的測試列（本人）→ 0 筆（正確擋下）；剛建立的測試列
（本人）→ 1 筆（正確允許）；剛建立的測試列（非本人 uuid）→ 0 筆
（既有 ownership 檢查未被本次修改破壞）。此法比 Phase BH 單純插入
測試列驗證讀取端更進一步，可直接驗證寫入端的 RLS 邏輯本身。

**既有產品決策的推翻須先確認**：`ReviewForm.tsx` 存在明確文案「評價
一經送出無法編輯，只能刪除重發（避免竄改）」，是使用者刷新頁面就會
看到的現行設計，非程式碼內部註解。BI.b 指令書要求為 `school_reviews`
新增編輯窗，直接與此文案矛盾。發現此類「指令書要求 vs. 現行明確產品
文案」衝突時，比照既有「spec 前提與 git 狀態不符須停下確認」的原則，
同樣須經 Lily 明確確認才能推翻，不能默認指令書優先於已上線的產品
決策逕自覆蓋——即使兩者都出自同一位需求方，文案本身代表的是「使用者
已經看到並可能依賴的行為承諾」，與程式碼內部的技術性假設不同，值得
額外一次確認。本輪經確認後才將該文案改寫並實作編輯窗。

**沿用既有表單元件的兩種模式**：`listings` 沿用既有 `PostModal`（原本
就是「建立」的彈窗容器），擴充 `BoardForm`/`PostModal` 接受
`editingListing` prop，同一個 modal 依有無帶入既有資料切換
insert/update 與標題文字；`school_reviews` 因 `ReviewForm` 原本是頁面
內嵌表單（非 modal），改在 `ReviewList.tsx` 內以本地 state 切換，點擊
「編輯」時在該則評價卡片下方就地展開帶入既有資料的 `ReviewForm`——
兩種呼叫端各自沿用自己既有的 UI 慣例，而非為了統一而勉強套用同一種
觸發機制。

## PAT-171 [CORE_IMMUTABLE]: 費用/族群標籤零臆測原則 + 跨檔案搬移前必查目標檔案現況（防重複資料）

**背景**：Phase BJ 為德文學習分類新增費用篩選、調整族群標籤，並將
DeutschAkademie 從 `german_learning.json` 轉移至 `schools.json`。

**BJ.a 費用標籤零臆測**：47 筆資源中，僅 8 筆原文明確提及費用性質
（`tags` 含「付費」或 `summary`/`detail` 明確寫出「免費」「付費」「訂閱制」
等字眼）：1 筆 free（Vorleser.net「免費有聲書平台」）、7 筆 paid（Udemy／
Lingoda／italki／AmazingTalker／Deutsch mit Marija／lingoni German／
GermanSkills）。**其餘 38 筆一律標為 `unknown`（未標示），即使常識判斷
該服務實際上是免費或訂閱制（如 Coursera/edX/Busuu/Memrise 現實中皆為
freemium），只要原文文字本身沒有明確寫出，就不得依外部知識推斷**——
這是本 PAT 的核心規則，避免「查證」淪為「憑印象猜測」。UI 設計上，
費用篩選預設值「任意」與新增的「未標示」選項皆不得讓 unknown 資源從
畫面消失（預設顯示全部，「未標示」讓使用者可主動篩選查看)。

**BJ.b 族群標籤命名 vs. 資料推導分離**：「初學者」更名為「入門」純屬
label 文字更動,既有 5 筆人工標註資料不變動、不重新判斷。新增的
「中級」「高級」則採**機械式推導**：依既有已查證的 `level`（CEFR）
欄位,level 含 B1/B2 → 中級、含 C1/C2 → 高級，同一資源可能橫跨多個。
這不是臆測——是對已驗證事實的單純轉換規則，全程可追溯、可重現，套用
範圍為全部 46 筆（新增後入門 5／中級 42／高級 21，皆非空篩選項）。與
BJ.a 的「零臆測」原則不衝突：BJ.a 是「原文沒寫就不標」，BJ.b 的
CEFR 推導是「原文已有的等級資料，換一個角度呈現」，兩者判斷基礎不同
（前者是新增一個原文未提供的事實，後者是重新組織原文已提供的事實）。

**BJ.c 搬移前必查目標檔案現況**：指令書假設 DeutschAkademie「尚未」
存在於 `schools.json`（用詞為「轉移至」），但實際查詢發現該校**已有
3 筆舊資料**（München/Berlin/Hamburg，來自更早的 Phase，注記為
「telc 官方授權考場四城之一」，`website` 皆為 null）——這是與本輪
「7 分校完整事實」框架不同的另一個查證脈絡（telc 考場授權 vs. 分校
版圖），兩者不衝突但資訊粒度不同。此差異在完成第一版搬移（新建 7 筆
獨立條目）後才被發現造成 id 重複，緊急回退重做。**教訓：任何「將 X
搬移/新增至 Y 檔案」的操作，執行前必須先查詢 Y 檔案是否已有 X 的既有
記錄（用實體名稱關鍵字 grep，不能只看指令書措辭是否暗示「不存在」）**，
避免產生重複 id 或覆蓋既有查證脈絡。經請示後採用「保留既有 telc 事實
語句，將本輪新查證的 1999 年創立／7 分校／定價事實併入既有 note 欄位」
的增強式合併，而非整批重寫或並列兩套獨立資料。

**BJ.c 分校資料結構**：沿用既有 Humboldt-Institut Berlin/Konstanz 先例
——**每個城市各自一筆獨立 school 條目**（同一 `id` 前綴＋城市 slug，
如 `deutschakademie-berlin`），而非發明新的「多分校陣列」欄位；每筆
`note` 皆列出全部分校城市供互相參照，同一城市內有多個地址（柏林
Wittenbergplatz／Alexanderplatz）不拆成兩筆，只在 note 內註明。7 個
所需城市（Berlin/München/Hamburg/Frankfurt/Stuttgart/Köln/Leipzig）
恰好全部已有既有城市天際線 SVG（因這些城市早已有其他學校使用同一
`city_key`），零新繪圖需求。

**BJ.c 46 筆同步排查方法論**：判斷標準非「是否收費」而是「是否為
與 schools.json 既有 Sprachschule 同構的實體多城市分校語言學校」——
schools.json 的 schema 本身以城市為錨點（`city`/`city_key` 驅動天際線
呈現），故即使 Lingoda（真人小班/一對一線上課程，付費）、italki／
AmazingTalker（付費家教媒合平台）等在「收費」意義上與 DeutschAkademie
相似，只要原文未提供其特定德國城市的實體分校地址，就不具備可對應
`city_key` 的必要條件，不應勉強指派一個城市造成資料失真，故不轉移。
排查結論：46 筆中無其他符合轉移條件的案例，全數維持於
`german_learning.json`。

## PAT-172 [CORE_IMMUTABLE]: 統計數字選 COUNT(*) 而非序號欄位的判斷依據——序號單調遞增不等於無縫隙保證

**背景**：Phase BK 為「支持本站」頁新增「目前已註冊人數」統計卡，需要
在 `profiles.registration_seq`（既有序號欄位）與即時 `COUNT(*)` 之間
選擇資料來源。

**查證過程**：讀 `deleteAccount.ts` 全文確認本站帳號刪除流程（PAT-94/95）
自始至終只對既有列做 UPDATE（匿名化 display_name/avatar_url + 記錄/
清除 `deletion_requested_at`），**沒有任何程式碼路徑會真正刪除
profiles 列**。查詢當下資料庫 `COUNT(*)=8`、`MAX(registration_seq)=8`、
`MIN=1`、無 NULL——序號完全連續無缺口，此刻兩種算法結果相同。

**判斷依據**：即使目前無缺口，`MAX(registration_seq)` 的正確性仍
依賴「序號欄位所在的列永遠不會被移除」這個假設——而這個假設只在
「透過本站既有程式碼流程操作」的前提下成立，若有 Lily 或治理端透過
Supabase 後台/直接 SQL 手動刪除某一列（略過本站的匿名化流程），
序號會出現缺口，`MAX(registration_seq)` 就會高估真實人數，且沒有
偵測機制能及時發現這個高估。`COUNT(*)` 則在任何情況下都是當下資料庫
的即時真實列數，不依賴任何流程假設。**結論：即使查證當下兩者結果
相同，只要存在「非既有流程路徑可能繞過」的可能性，就該選擇不依賴
該假設的算法**，而非以「現在沒問題」為由選擇較脆弱的一方。查詢方式
沿用既有 `useLikes.ts`/`useFollow.ts`/`useBadges.ts` 建立的
`.select('id', { count: 'exact', head: true })` + `fetchWithRetry`
慣例，不新建查詢風格。

**適用範圍**：此判斷框架不限於本次「已註冊人數」統計，未來任何需要
「目前總數」的場景，若候選資料來源之一是「遞增序號欄位的極值」、
另一是「即時 COUNT」，優先評估序號欄位所在列是否有任何刪除可能性
（即使目前查無缺口），而非僅看當下數字是否吻合就直接採用序號欄位。

## PAT-173 [CORE_IMMUTABLE]: 「純 React state = 不重複觸發」的隱含前提——觸發來源本身可能重複觸發，需獨立追蹤「本次已處理」旗標

**背景**：Phase BL 要求 `SkipLoginConsentPrompt.tsx` 的關閉狀態「僅整頁
載入時觸發,SPA 內部導覽不重複彈出」。執行前查證發現，該元件的開關
狀態（`open`）本就是純 `useState`，零 `localStorage`/`sessionStorage`，
且掛載於 `App.tsx` 根層級（`<Routes>` 外層），初步判斷已完全符合目標
行為——這個判斷**不完整**，實際瀏覽器測試才發現真正的缺口。

**缺口成因**：此元件不是直接被使用者互動觸發，而是監聽
`window.dispatchEvent(new Event('onboarding-modal-closed'))` 這個
CustomEvent（由 `Home.tsx` 內的 `OnboardingModal` 關閉時呼叫）。
`Home.tsx` 本身掛在 `<Routes>` **內**，SPA 導覽離開又切回「/」時會
重新掛載，其 `onboardingOpen` state 重置為初始值——代表同一次頁面
載入期間，使用者只要「切走再切回首頁」就能讓 `OnboardingModal` 重新
出現一次，而使用者若再次關閉它，`onboarding-modal-closed` 事件會
**再次觸發**，若監聽端只憑 `open` 這顆 state 判斷「該不該打開」，
就會被這次遲來的事件重新打開——即使 `open` 本身確實是純 state、確實
不寫入任何 Storage，仍然無法防止「同一個純 state 元件被同一個可重複
發送的事件多次觸發」。

**教訓**：驗證「元件狀態是否為純 React state、不持久化」只回答了
「重新整理後會不會重置」，沒有回答「同一次頁面載入內，觸發來源本身
是否可能重複發送信號」。這兩者是獨立的問題，前者由「元件掛載位置＋
state 初始值」決定，後者取決於「觸發事件的上游來源是否可能在同一次
載入內重複發生」——本例的上游來源（`Home.tsx` 的 `OnboardingModal`）
本身會在 SPA 導覽下重新掛載並重新觸發事件，這是查證階段容易漏掉的
「觸發來源本身的生命週期」，而非「本元件自身的 state 管理方式」。

**修正方式**：新增獨立的 `dismissedThisLoad` state（同樣是純
`useState`，不持久化），使用者第一次關閉時設為 `true`；事件處理常式
改為同時檢查 `!dismissedThisLoad`，忽略後續任何重複觸發的事件，直到
真正整頁重新整理（`dismissedThisLoad` 隨 App 重新掛載重置為 `false`）。

**驗證方法論**：本輪未僅憑程式碼審閱＋「pure state 理論上會如何表現」
的推理就下結論，而是實際在瀏覽器內重現「關閉→SPA 導覽離開→導覽回
首頁→再次關閉」的具體操作序列，才發現修正前的版本會重新彈出；也在此
過程中發現 `mcp__Claude_Browser__navigate` 的 `force: true` 參數在本
環境下不保證是真正的整頁重新整理（`performance.getEntriesByType
('navigation')[0].type` 顯示不可靠，`performance.now()` 數值遠大於
理論上剛載入的頁面應有的量級），改用 `window.location.reload()` 才能
確保測試的是真正的整頁重新整理情境，此為本輪驗證方法論的副產品發現，
供未來需要區分「SPA 導覽」與「真正整頁重新整理」的測試情境參考。

## PAT-174 [CORE_IMMUTABLE]: 指令書聲稱「已確認」的技術性 bug 若無法重現，須據實回報並請示，不得憑空修復或悄悄略過

**背景**：Phase BM 指令書聲稱找房頁面出現兩組重複卡片（技術性重複，
非編輯判斷），並指定根因排查方向（JSON 重複列／React key 碰撞／篩選
邏輯邊界)。

**查證過程**：逐一排除所有可能根因——`housing.json` 20 筆 id 全數
唯一、`RecommendationCategory.tsx` 單一 `.map()` 迴圈搭配
`key={item.id}`、跨檔案 grep 確認 5 個具名平台未出現在其他分類 JSON、
瀏覽器實測預設狀態（20 張卡片）與費用/租期/族群篩選的多種組合
（含專門測試「合租」篩選涵蓋全部 5 個具名平台的情境）均未發現任何
重複渲染。全站其餘 10 個分類的 JSON 亦逐一 script 化檢查，id 與
title 皆無重複。

**處理方式**：在窮盡合理排查手段仍無法重現指令書所述問題時，**沒有
逕自捏造一個「修復」去對應一個找不到的 bug**，也沒有悄悄略過此項
不提——而是如實使用 AskUserQuestion 向 Lily 說明已排查的範圍與方法，
詢問她實際觀察到重複卡片的具體情境（本地 dev server／部署站台／
特定篩選組合等），取得「確實在本地 dev server 看到」的回覆後，記錄
於報告的 DEBT 段落，作為待補查的已知缺口，而非虛報「已修復」或
沉默略過「找不到，所以當作沒發生」。

**原則**：查證任務的產出不是「一定要交出一個修復」，而是「誠實反映
查證後的實際狀態」。若指令書的前提與可觀察到的實際狀態不符（無論是
BJ.c 案例的「schools.json 已有舊資料」還是本例的「找不到重複渲染」），
正確反應永遠是向需求方核實，而非以任何方向的猜測掩蓋落差——猜測著
去「修」可能製造出全新的、真實不存在的問題；猜測著略過則會讓真正的
問題（若確實存在於某個我未能重現的情境）繼續被誤認為已解決。

## PAT-175 [CORE_IMMUTABLE]: 「找不到渲染層 bug」不等於「沒有問題」——需回頭檢視資料正確性層面

**背景**：Phase BM 排查找房頁「重複卡片」報告時,方向設定為渲染層
bug（同一筆資料被畫兩次),窮盡排查後確認渲染層完全乾淨,如實記錄
「無法重現」。Phase BN 重新定義問題本質：使用者感受到的「重複」並非
技術性渲染錯誤,而是資料正確性問題——`wohnungsboerse.net` 與
`ImmobilienScout24`、`Studenten-WG` 與 `HousingAnywhere` 因企業併購
已成為同一集團旗下服務,但 `housing.json` 仍將其收錄為獨立、看似
互相競爭的平台,使用者點開兩張「不同」卡片後才發現背後是同一家公司
——這種「重複感」是內容審核疏漏,不是程式錯誤,渲染層自然找不到任何
技術性根因。

**教訓**：使用者回報的「症狀」（如「重複」「一樣的東西出現兩次」）
描述的是**體驗結果**,不等於**問題所在層次**的判斷。渲染層 bug 與
資料正確性問題會製造出使用者觀感上相似的「重複/混亂」症狀,但根因
與修復方式完全不同：前者是程式邏輯錯誤（key 碰撞、篩選邊界、資料列
重複),後者是內容查證疏漏（同一實體被誤登記為多個獨立項目)。窮盡
排查某一層次仍找不到根因時,正確做法不是「反正找不到,問題大概不
存在」,而是**回頭檢視問題是否根本發生在另一個層次**——本例即是從
「渲染邏輯」轉向「內容事實查證」才找到真正根因。

**驗證方法論**：判斷「兩個看似獨立的品牌背後是否同一集團」須用
WebFetch 直接查證網域**現況**（是否仍獨立運作,或已 301 導向母集團
網域),不能只憑「查到收購新聞稿」就推定網站現況。本輪三次 WebFetch
皆直接證實 301 導向鐵證（wohnungsboerse.net→immobilienscout24.de、
studenten-wg.de→housinganywhere.com、zwischenmiete.de→
housinganywhere.com/de/404，末者更帶有品牌專屬 utm_source 追蹤參數
「Zwischenmitte」，成為判定其隸屬 HousingAnywhere 集團的直接證據)，
比僅憑新聞稿或商業資料庫記錄更能反映即時事實；同時也發現指令書自身
對 `wohnungsboerse.net`「目前官網仍正常運作、非導向 ImmoScout24」的
...
## PAT-179 [CORE_IMMUTABLE]: 套用「結論先行＋精簡列點」三層格式（PAT-152）精簡帶獨立信心分級徽章的內容時，徽章視覺系統須保留精簡版，不得因追求簡潔而消失於摘要層

**背景**：Phase BQ 把 Phase BP 建立的 14 張簽證卡從「收合狀態僅顯示
標題」改版為「收合狀態顯示結論＋條列關鍵資訊＋財力精簡一行」，比照
`RecommendationCategory.tsx` 既有的 `summary`/`points`/`detail` 三層
格式（PAT-152）。但簽證卡的財力欄位不是單純文案，而是帶有 PAT-176/
Phase BP 建立的四級信心分級視覺徽章（官方／替代驗證-高／替代驗證-中／
缺口）——若照搬 PAT-152 的作法（把長文字改寫成一行摘要），會在精簡
過程中不慎把徽章一併省略掉，變成摘要層看起來像是「肯定的官方數字」，
使用者要點開展開區才會發現原來是低信心的估算值，這正是指令書
BQ.b 明確禁止的情況（「不得省略『替代驗證』字樣直接當成官方數字
呈現」）。

**教訓**：把「結論先行＋精簡列點」格式套用到**帶有獨立視覺信號系統**
（confidence badge、狀態標籤等）的內容時，精簡的對象只能是「敘述文字」，
不能連同視覺信號系統一起精簡掉——正確作法是為該信號系統建立一個
**同色系、同圖示、僅文字縮短**的精簡版（本例 `CONFIDENCE_LABEL_SHORT`，
如「替代驗證 · 信心：中」精簡為「替代·中信心」，但保留 `~` 圖示與
金色系底色），讓摘要層仍可用顏色/圖示在 5 秒內辨識信心層級，完整說明
與計算過程才移入展開區（`CONFIDENCE_LABEL`／`ConfidenceEntry.quote`
原樣保留，未刪減）。判斷原則：三層格式的「精簡」原則作用於**冗長度**，
不作用於**已建立的視覺區隔語意**——兩者是正交的兩個維度，精簡前者
不應連帶犧牲後者。

## PAT-180 [方法論]: 建立跨內容域對照表（如赴德指南板塊↔資源分類）須以「同一實體被兩邊具體引用」為依據，同類別government office 表面相似不等於同一機關

**背景**：Phase BR.d 建立赴德指南 8 個板塊與資源 11 個分類（含子板塊）
的人工對照表。初步直覺曾考慮把「落地指南」的「確認能否辦戶籍／戶籍
登記」步驟對應到資源「外事局」分類——兩者表面上都是「政府機關辦理
手續」，但深查後發現：「戶籍登記」（Anmeldung）辦理機關是 Bürgeramt/
Einwohnermeldeamt，而資源「外事局」分類實際收錄的是 Ausländerbehörde
（如柏林 LEA、慕尼黑 KVR–Ausländerbehörde，居留許可核發機關）——
兩者是完全不同的德國行政機關，若建立此對應會誤導使用者以為「戶籍
登記」可以在外事局資源頁面找到辦理入口。反之，「延簽流程」的「線上
預約 Termin」步驟與「離開指南」的「通知移民局」步驟，皆在
`official_sources` 明確引用「Berlin 移民局／Ausländerbehörde」——與
資源外事局分類收錄的機關是同一實體，這才是可成立的對應。

**教訓**：跨內容域建立對照表時，正確的判斷依據是**兩邊是否具體引用
同一實體**（同一品牌、同一工具、同一官方機關網址），而非**同一表面
類別的字面相似**（都叫「政府機關」「金融服務」「保險」等）。表面類別
相同、實體不同是常見陷阱——尤其德國行政體系存在多個外觀相似但職權
不同的機關（Bürgeramt 戶籍登記 vs Ausländerbehörde 居留許可 vs
Finanzamt 稅務），命名或功能領域接近但辦理內容完全不通用。**驗證
方法**：查該板塊某個具體步驟的 `official_sources`／內文是否明確點名
與資源分類收錄項目相同的品牌或機關連結（如本例 Fintiba/N26/
Rundfunkbeitrag/Berlin Ausländerbehörde 等皆兩邊逐字比對確認），
找不到具體引用重疊的內容一律不建立對應，寧可在報告列為「查無對應」
也不可勉強套用表面相似的分類湊數。

## PAT-181 [CORE_IMMUTABLE]: 同一結果區塊內顯示多種貨幣/數字時，千分位分隔符須統一一套慣例，不得依貨幣「原生語系」各自套用 toLocaleString 語系代碼

**背景**：Phase BS 財力估算器同時顯示歐元總額與台幣換算金額。初版
分別用 `toLocaleString('de-DE', ...)`（歐元，德國原生語系，千分位用
句點、小數點用逗號）與 `toLocaleString('zh-TW', ...)`（台幣，千分位用
逗號）各自「就地取材」對應貨幣的原生語系代碼，兩個數字並排顯示於
同一結果區塊時，€2.424（歐元，句點千分位）與 NT$86,052（台幣，逗號
千分位）分隔符號不一致，容易被慣用逗號千分位的台灣使用者誤讀（把
€2.424 看成「2.424 歐元」的小數，而非 2,424）。瀏覽器人工測試時才
發現此問題，非程式邏輯錯誤（加總計算本身完全正確），純屬顯示層級
的在地化疏忽。

**教訓**：`toLocaleString(languageTag)` 的語系代碼選擇不該以「這個
數字是哪國貨幣」為依據，而該以「實際閱讀這個畫面的使用者慣用哪種
數字書寫慣例」為依據——本站使用者是台灣人，無論顯示的是歐元、
台幣、或任何貨幣，千分位分隔符都該統一用使用者慣用的逗號慣例
（`en-US`/`zh-TW` 皆可，只要全站/同一畫面一致即可），不要讓每個
金額各自「入境隨俗」套用該貨幣原生語系的格式化慣例。同一結果區塊
內若顯示多個數字（無論同貨幣或跨貨幣），必須用同一套格式化函式或
至少同一組分隔符號規則，避免使用者需要在同一畫面內切換兩套讀數
...
該 skill 後續補充第 8、9 節（治理端驗證強度同樣依風險分級、避免小
改動重啟完整 Agentic 工作階段）與用量分級速查表（同倉庫 commit
`9702920`）。

**再追加備註**：另新增 `skills/collaborating-with-lily/SKILL.md`
（同倉庫 commit `3e3c54d`），記錄與 Lily 協作觀察到的溝通風格、
決策習慣、技術偏好，以及需要主動留意的易錯模式（供未來對話快速
校準預設行為），不在本檔重複全文。

**第三次追加備註**：另新增 `skills/mandatory-toolkit-sync/SKILL.md`
（同倉庫 commit `bb04a3d`），規定每次新 Phase pre-flight 第一步須先
`git pull` 同步工具包、依技能描述比對是否命中既有原則，命中/未命中
皆須於報告註明。同時查核本機技能載入機制：未發現 `~/.claude/skills/`
或任何 `settings.json`／plugin 設定檔引用技能掃描路徑，故無法確認
`web-scaffold-toolkit/skills/` 是否已被原生機制自動偵測；此設定細節
已於當次完成報告向 Lily 提出確認請求，本檔不重複貼全文。

## PAT-183 [方法論]: 指令書附件與敘述不符時須實際逐項核對而非假設涵蓋；必要必填欄位（如 url）缺值時，可對「客觀可查證的公開事實」（官方網址等）以治理端 web 查證補齊，不算違反零虛構原則，僅對主觀/未公開事實（費用細節、政策內容）才需標示未確認

**背景**：Phase BX 指令書敘述「德文學習資源補充清單（10 筆）」並具體
點名 Viki.com／allango／`Übungsgrammatik für die Grundstufe`／
`Übungsgrammatik für die Mittelstufe`／Deutsches Geplapper 等項目，
但同批附上的「德文學習資源分類清單」文件經逐類別核對後，完全不含
上述任一具名項目（該文件是另一份 11 類、約 90 筆資源的清單，且新增
「語言交換」「YT影片」兩大類，與指令書「延續既有 9 子板塊」的前提
互相矛盾）。用 AskUserQuestion 詢問後，Lily 第一次回答只是把指令書
原文再貼一次（未提供實際逐筆資料），第二次追問更明確指出缺少的是
「url／等級／費用／一句話摘要」等具體欄位後，Lily 明確選擇「僅用
指令書敘述本身的文字」，忽略不符的附件。

**教訓（附件與敘述不符的處理方式）**：發現指令書敘述與附件內容不符時，
不能假設「附件大概涵蓋了敘述提到的東西」就直接動工，須逐項核對
（例如本次是逐一搜尋具名項目是否出現在附件的 11 個類別中）確認落差
範圍後才用 AskUserQuestion 呈現落差，並在使用者第一次回答未解決落差
時繼續追問到具體可執行為止，而非在含糊回答後就直接動工。

**教訓（url 等必填欄位缺值的正確處理，屬 PAT-171 的邊界案例）**：
`Recommendation.url` 為必填欄位，指令書敘述本身對 10 筆項目全數未
提供實際網址；但 Viki.com／allango／Hueber 兩本書／Deutsches
Geplapper 皆為可獨立查證身分的真實公開實體，其官方網址本身是客觀、
穩定、可驗證的事實（如同已知 Duolingo 官網是 duolingo.com），並非
臆測費用/等級/政策等主觀細節。本輪對這 5 筆以治理端 WebSearch 查證
官方網址與少數可公開查證的客觀事實（如 allango 承接 Klett Augmented
App、2024-08-31 停用日期、Viki 提供免費+Viki Pass 付費雙軌），視為
「查證」而非「虛構」，比照指令書本身已示範的 allango 治理端查證作法
（先於本 Phase 由治理端查證 allango 停用宣稱）；但敘述中完全未提及
名稱、無法查詢任何線索的其餘 4 筆（文法 2 筆、閱讀 1 筆、詞彙 1 筆）
則誠實記錄為缺失資料，不寫入 `german_learning.json`，不得為湊「10 筆」
數字而杜撰條目。指令書本身「10 筆」與其逐項拆解加總（僅得 9 筆，見
完成報告）也存在落差，同樣誠實記錄不強行湊數。

## PAT-184 [方法論]: 來源文件自稱「已去重」不可直接信任，須以現有資料庫實際內容（URL/標題字面比對）逐筆覆核；新增子板塊分類與既有資料的分類調整屬不同範疇的決定，不應在同一輪順帶擴大範圍

**背景**：Phase BY 來源文件（約 78 筆、11 類）自稱「本輪重複項目
（Goethe-Institut、DW Learn German、deutsch.info、Duolingo等27項）
已自動合併，不重複收錄」。治理端先發現 `Deutsch mit Marija`、
`GermanSkills.com` 兩筆確實仍存在於既有資料（Phase BE/BJ 建立)卻未被
文件自己的排除清單涵蓋，circumstantial 已足以懷疑文件自稱不可信；
逐筆以 URL／標題與現行 `german_learning.json` 全量比對後，實際發現
45 筆重複（文件自稱僅 27 項排除，落差達 18 筆），涵蓋 11 類中的 9 類
（僅「口說」「YT影片」兩個新設分類零重複，因既有系統本無這兩類）。

**教訓（覆核方法）**：來源文件的「已去重」「已排除」等自我宣稱只能
當作起點，不能當作結論；正確作法是不論文件宣稱與否，一律用實際資料
庫內容（URL 為主、標題為輔，因同一資源常見不同分頁路徑或子網域）
逐筆機械比對，才能發現文件自己遺漏的重複，不受文件自身盤點範圍
所限。

**教訓（新分類與既有資料重新分類屬不同決定，不應順帶擴大範圍）**：
文件將 `ConversationExchange`／`HelloTalk`／`Tandem`／`HiNative`／
`italki`／`AmazingTalker`／`Lingoda` 等 7 筆既有資料重新歸類到新設的
「語言交換」分類下，但這是文件自身重新整理的結果，不是本 Phase 指令書
交付的任務——指令書僅要求「依比對後的淨新增清單逐筆轉錄」，並未要求
連動修改既有資料的 `board` 欄位。本輪維持既有 7 筆的原有子板塊歸屬
不變，「語言交換」分類本輪僅新增 1 筆真正新資源（slowly.app），並在
完成報告向 Lily 提出「既有 7 筆是否要一併重新歸類」作為獨立待決定
事項，不在本輪未經確認下自行擴大異動範圍。

## PAT-185 [DEPRECATE_MARK]: Phase CA「未標示」費用全數改標「混合」是 Lily 知情、一次性授權的零臆測原則（PAT-171）例外，非新通則——下批資料證據不足仍預設標「未標示」

**背景**：Phase CA 指令書明確要求將 `german_learning.json` 中所有
`fee: "unknown"`（未標示）的資源改標為 `fee: "mixed"`（混合），即使
沒有實際證據支持該資源確實為混合收費模式。這與 Phase BJ.a 建立、
PAT-171 記錄的零臆測原則（無證據不得標註具體費用類型，只能標
unknown）直接衝突。指令書本身已明確標註這是「Lily 明確知情、一次性
授權的例外決定」，經治理端提出風險說明後 Lily 仍選擇執行，且明確
聲明「這不是新的通則，不代表未來批次資料的『未標示』情況都要比照
處理」。

**受影響筆數與清單（共 36 筆，全數來自 `german_learning.json`）**：
`gl-goethe-tw`、`gl-dw-learn-german`、`gl-deutsch-info`、`gl-duolingo`、
`gl-scu-deutsch`、`gl-busuu`、`gl-grammatiktraining`、
`gl-lerngrammatik`、`gl-mein-deutschbuch`、`gl-deutschlernerblog`、
`gl-leo`、`gl-dwds`、`gl-pons`、`gl-duden`、`gl-wortschatz-leipzig`、
`gl-deepl`、`gl-languagetool`、`gl-tagesschau`、`gl-zeit-online`、
`gl-euronews-de`、`gl-kindernetz`、`gl-grimmstories`、
`gl-languageguide`、`gl-easy-german`、`gl-slow-german`、`gl-anja`、
`gl-deutsch-dianxin`、`gl-anki`、`gl-memrise`、`gl-hinative`、
`gl-conversationexchange`、`gl-hellotalk`、`gl-tandem-app`、
`gl-goethe-zertifikat`、`gl-osd`、`gl-testdaf`。全站範圍確認：僅
`german_learning.json` 的 `fee` 欄位使用「未標示」標記；`housing.json`
的 `fee_status`（型別含 `unknown`→「未知」）與 `career.json` 的
`career_fee`（型別無 `unknown` 值）皆無任何實際資料使用該值，故本次
異動範圍確認僅此一份檔案。逐筆核對這 36 筆的 `detail`／`points`／
`note` 欄位，未發現任何一筆曾明文寫下「原文未提及費用」一類的查證
記錄文字——`unknown` 原本就是欄位層級的沉默預設值（Phase BJ 建立時
「其餘一律 unknown」），非逐筆人工記錄的查證結論，故本輪不需要另外
調整或移除任何 detail 文字，沒有語意衝突需要處理。

**後續影響（篩選 UI）**：`GermanLearningFee` 型別本身保留 `unknown`
不刪除（見 `recommendation.ts` 的 `GERMAN_LEARNING_FEE_ORDER`），因
未來批次仍可能需要用到；但篩選下拉選項改為依 `items` 實際使用的值
動態產生（`GermanLearningBoard.tsx` 的 `feeOptions`），現行 81 筆
零筆使用 `unknown`，故「未標示」選項自動消失，非留著一個恆為空的
選項；若之後任何一批資料又合法使用 `unknown`，選項會自動重新出現，
不需要手動改回。

**教訓（例外決定的顯式標記，見 [[collaborating-with-lily]]／
[[token-efficient-governance]]）**：這類「使用者明確要求覆蓋既有品質
原則」的請求，正確流程是先攤開風險講清楚、取得明確確認後才執行，且
執行時必須在專案記錄中明確標註「這是一次性例外，非新通則」——本則
記錄本身就是這個標註，供未來任何一筆「混合」標籤被使用者回報與實際
不符時，可直接判斷是本次批次調整所致（一次性治理決定），而非資料
查證本身出錯，也避免下一次類似情境被誤判為「上次都這樣做，這次應該
也可以」而自動延伸適用範圍。下次遇到費用證據不足的資源，預設回歸
標為「未標示」，除非 Lily 再次明確要求覆蓋。

**追加備註**：Phase BX/BY/CA 過程中萃取出的 4 條跨專案通則，已寫入
`web-scaffold-toolkit` 的 `GOVERNANCE.md` 第 10–13 章（同倉庫 commit
`ffb2a9b`）：raw.githubusercontent.com 快取延遲異常時改用 jsDelivr
備援、大型來源文件與指令書不符時的跨輪次留痕方式、來源文件自稱已
完成的檢查須機械化全量比對而非抽查、使用者授權推翻既有品質原則的
例外須顯式治理標記。本檔不重複貼全文。

## PAT-186 [方法論]: 中英混合標題排序須實測驗證 locale 選擇而非預設信任程式輸出；卡片排序範圍限定在「有子板塊結構」的分類，不擅自擴及扁平式分類

**背景**：Phase CB.b 要求德文學習（11 子板塊）與 DACH 實習/求職
（3 子板塊）內的卡片依標題排序，明確要求「使用 locale-aware 排序
（不使用預設 Unicode code point 排序）」且「執行前用幾筆已知資料
人工核對排序結果是否符合直覺預期，不僅信任程式輸出」。

**實測結果**：以德文學習「文法」（11 筆，含中英混排與 Ü 變音符號）與
「綜合」（11 筆，中文/英文標題並存）兩組已知資料實測
`localeCompare(b, 'zh-Hant')` 與 `localeCompare(b, 'de')`，結果一致且
符合直覺：CJK 標題整組排在前（依筆畫/內碼），其後英文標題依大小寫
不敏感字母序排列，`Übungsgrammatik` 正確落在 U 附近（未被變音符號
拆散到不直覺的位置）。改用不帶 locale 參數的預設 `localeCompare`
（即讓瀏覽器套用執行環境的系統 locale）在 Node 測試環境恰好也得到
相同結果，但**不能保證所有使用者瀏覽器的預設 locale 都一致**，故
最終選擇顯式指定 `'zh-Hant'`，確保無論使用者瀏覽器語言設定為何，
排序行為在本站（繁體中文為主要語言）都是決定性、一致的。

**排序時機決策**：選在 `GermanLearningBoard.tsx`／`CareerBoard.tsx` 既有
的 `visibleItems` useMemo 內（篩選之後、渲染之前）追加 `.sort()`，
不改動 `german_learning.json`／`career.json` 本身的條目順序——符合
「依現行程式碼架構選擇較小改動方式」的指示，也保留資料檔既有的
「概略依來源文件章節分組」慣例（PAT 記錄用途），排序純粹是顯示層
關注點。

**範圍決策（未擴大套用至扁平式分類）**：指令書明確將「其餘 9 個扁平式
分類是否套用同一排序」列為開放判斷，本輪判斷**不主動擴大**——理由：
(1) 扁平式分類（找房/金融/交通等）目前條目數量普遍遠少於德文學習
11 子板塊的量級，字母序帶來的可尋性效益不明顯；(2) 指令書字面只點名
「子板塊」（有兩層導覽結構的分類），擴大套用屬另一個範圍決定，若
真要做應該是獨立一個 Phase（可能連動考慮這些分類是否也要排序、或
維持現有的「策展手動排序」慣例），不在本輪未經確認下順帶處理，
呼應 PAT-184 established 的「新範疇擴大需獨立列為待決定事項」原則。

## PAT-187 [CORE_IMMUTABLE]: 首頁 Portal 卡片導向的外部工具，須先經站內子頁面轉一手（比照資源分類頁版型），不得從首頁卡片直接外連

**背景**：Phase CB.a 第一版曾直接在首頁 Portal 卡片上設定外部連結
（`target="_blank"` 直接指向 `game.stayinde.de`），已完成實作、通過
驗證並 push。同一 Phase 隨即收到修正指令書，明確要求改為兩層架構：
首頁卡片導向站內新路由 `/pretrip-challenge`，真正的外部連結收在該
子頁面內部的卡片上。修正後的版本已覆蓋掉第一版的實作（同一分支追加
commit，未強行保留兩種寫法）。

**教訓（架構決策，非文字微調）**：「外部連結卡片」與「內部子頁面+
內部卡片轉外部連結」是兩種不同的架構決策，不是同一設計的措辭調整——
第一版技術上完全可運作（typecheck/build/驗收標準當時皆通過），但不
符合 Lily 對「首頁卡片一律導向站內」的產品慣例期待。收到明顯的架構
級修正指令時，應直接以新架構取代舊實作（同分支追加 commit），而非
嘗試「盡量保留舊程式碼、只加一層包裝」——後者會讓 `PORTAL_ITEMS` 同時
存在 `to`／`href` 兩種分流邏輯却只有一筆資料用得到 `href`，是不必要
的複雜度殘留。

**可複用的產品慣例（供未來新增外部工具入口參考）**：首頁 Portal 卡片
應永遠是站內路由（`<Link>`），不應該有任何一張卡片直接帶
`target="_blank"` 外連；真正的外部連結收斂在對應的站內子頁面內部
（比照 `RecommendationCategory.tsx` 的「標題+卡片列表」版型，卡片上的
「官網 ↗」按鈕才帶 `target="_blank"` + `rel="noopener noreferrer"`）。
即使子頁面目前只有 1 筆資料，仍依此版型建立（本例：
`PretripChallenge.tsx` 內建固定陣列 `PRETRIP_TOOLS`，非讀取
recommendations JSON——因為這類治理端手動精選的少量入口不需要
使用者提交/篩選機制，不必也不應該掛進 `RECOMMENDATION_CATEGORIES`／
`DATA_MAP`／`target_category` CHECK 約束體系，那是給資源 11 分類用的，
語意不同），方便未來擴充不需重新設計頁面。
