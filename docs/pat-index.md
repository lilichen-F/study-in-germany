...
| PAT-186 | 方法論 | 中英混合標題排序須以已知資料實測驗證 locale 選擇（確認 `localeCompare(b, 'zh-Hant')` 對 CJK+英文混排與 Ü 變音符號皆給出直覺結果）才能信任，不可只信任程式輸出；排序邏輯放在既有 useMemo 篩選後、渲染前（顯示層關注點，不改資料檔順序）；卡片字母排序範圍限定在有子板塊兩層導覽結構的分類（德文學習/求職），不擅自擴大套用至扁平式分類，該範圍決定須獨立列出供使用者確認 |
